...
/*
	...
	Plugin URI: https://github.com/byteworksng/interswitch-webpay
	...
	Author URI: https://github.com/byteworksng
	License:           GPL-2.0+
...
add_action( 'plugins_loaded', 'chi_wc_interswitch_webpay_init', 0 );

function chi_wc_interswitch_webpay_init() {
	...
	class WC_Chi_Webpay_Gateway extends WC_Payment_Gateway {
		...
		public function __construct() {
			...
			//Actions
			add_action( 'woocommerce_receipt_chi_webpay_gateway', array( $this, 'receipt_page' ) );
			add_action( 'woocommerce_update_options_payment_gateways_' . $this->id, array( $this, 'process_admin_options' ) );

			// Payment listener/API hook
			add_action( 'woocommerce_api_wc_chi_webpay_gateway', array( $this, 'check_webpay_response' ) );
		...
		**/
		public function get_webpay_args( $order ) {
			...
			WC()->session->set( 'chi_wc_webpay_txn_id', $txn_ref );
	    ...
	    **/
	    public function generate_webpay_form( $order_id ) {
			...
			// before payment hook
            do_action( 'chi_wc_webpay_before_payment', $webpay_args );
		...
		**/
		public function check_webpay_response() {
			...
			if( ! empty( $_POST['txnref'] ) || ! empty( $_REQUEST['txnRef'] ) ) {
		        ...
		        $response       = $this->chi_webpay_transaction_details( $txnref, $total );
				...
				// after payment hook
                do_action('chi_wc_webpay_after_payment', $_POST, $response );
			...
			update_post_meta( $order_id, '_chi_interswitch_wc_message', $notification_message );
	 	...
	 	**/
		public function chi_webpay_transaction_details( $txnref, $total ) {
	    ...
	    **/
		public function display_transaction_id() {
			...
			if( get_query_var( 'order-pay' ) ) {
				...
				if( !isset( $_GET['pay_for_order'] ) && ( 'chi_webpay_gateway' == $payment_method ) ){
					$txn_ref =$order_id = WC()->session->get( 'chi_wc_webpay_txn_id' );
					WC()->session->__unset( 'chi_wc_webpay_txn_id' );
	...
	function chi_wc_interswitch_message() {
		...
		if( get_query_var( 'order-received' ) ) {
			...
			if( is_order_received_page() &&  ( 'chi_webpay_gateway' == $payment_method ) ) {

				$notification 		= get_post_meta( $order_id, '_chi_interswitch_wc_message', true );
				...
				delete_post_meta( $order_id, '_chi_interswitch_wc_message' );
	...
	add_action( 'wp', 'chi_wc_interswitch_message', 0 );
	...
	if ( version_compare( WOOCOMMERCE_VERSION, "2.1" ) <= 0 ) {
		...
		**/
		add_filter( 'woocommerce_currencies', 'chi_add_my_currency' );

		if( ! function_exists( 'chi_add_my_currency' ) ) {
			function chi_add_my_currency( $currencies ) {
		...
		**/
		add_filter('woocommerce_currency_symbol', 'chi_add_my_currency_symbol', 10, 2);

		if( ! function_exists( 'chi_add_my_currency_symbol' ) ) {
			function chi_add_my_currency_symbol( $currency_symbol, $currency ) {
			     switch( $currency ) {
	...
	**/
	function chi_webpay_plugin_action_links( $links, $file ) {
	    static $this_plugin;
	    ...
	    if ($file == $this_plugin) {
	        $settings_link = '<a href="' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=wc-settings&tab=checkout&section=wc_chi_webpay_gateway">Settings</a>';
	        array_unshift($links, $settings_link);
	    }
	    return $links;
	}
	add_filter( 'plugin_action_links', 'chi_webpay_plugin_action_links', 10, 2 );
 	...
 	**/
	function chi_webpay_testmode_notice() {

		$chi_webpay_settings = get_option( 'woocommerce_chi_webpay_gateway_settings' );

		$webpay_test_mode = $chi_webpay_settings['testmode'];
	...
	add_action( 'admin_notices', 'chi_webpay_testmode_notice' );
